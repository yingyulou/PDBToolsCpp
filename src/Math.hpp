...
{
    RowVector3d refCenterCoord = refCoordArray.colwise().mean();
    ...
    return {tarCenterCoord, rotationMatrix, refCenterCoord};
}
...
double calcRMSDAfterSuperimpose(const Matrix<double, Dynamic, 3> &refCoordArray,
    const Matrix<double, Dynamic, 3> &tarCoordArray)
{
    auto [tarCenterCoord, rotationMatrix, refCenterCoord] = calcSuperimposeRotationMatrix(
